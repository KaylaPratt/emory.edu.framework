...
/******************************************************
 * CLEANUP TASKS - Delete it all
******************************************************/
// Cleanup 
gulp.task('clean:public', function () {
    return gulp.src('*',{cwd: normalizePath(paths().public.root), read: false})
      //return gulp.src("./public/*")
      .pipe(clean({force: true}));
});

// Cleanup 
gulp.task('clean:compile:css', function () {
    return gulp.src('*.+(scss|less).*',{cwd: normalizePath(paths().source.css)})
      .pipe(clean({force: true}));
});
...
gulp.task('compile:css', function(){
    if(getConfig().cssPreprocessor === 'scss'){
      return gulp.src(normalizePath(paths().source.scss) + '/*.scss')
        .pipe(sass())
        .pipe(postcss([autoprefixer]))
        .pipe(gulpif(getConfig().productionOutput, cssmin()))
        .pipe(rename({
          suffix: '.scss'
        }))
        .pipe(gulpif(getConfig().productionOutput, rename({
          suffix: '.min'
        })))
        .pipe(gulp.dest(
          normalizePath(paths().source.css)
        ));
    } else {
      // Assume Less
    ...
    //return gulp.src('**/*.*',{cwd: normalizePath(paths().public.root)});
});
...
gulp.task('pl-copy:js', function () {
  return gulp.src('**/*.js', {cwd: normalizePath(paths().source.js)})
    .pipe(gulpif(getConfig().concatJS, concat(
      'application.js'
    )))
    .pipe(gulpif(getConfig().productionOutput, uglify()))
    .pipe(gulpif(getConfig().productionOutput, rename({
      suffix: '.min'
    })))
...
gulp.task('patternlab:build', gulp.series('clean:public', 'clean:compile:css', 'compile:css' , 'pl-assets', build));
...
function watch() {
  const watchers = [
    ...
    {
      name: 'CCSS',
      paths: [normalizePath(paths().source.css, '**', '*.(less|scss)')],
      config: { awaitWriteFinish: true },
      tasks: gulp.series('compile:css', 'pl-copy:css', reloadCSS)
    },
    {
